	...
	for !syncDone {
		...
		case ChallengeMessagePayload:
			...
			if response.Challenge != nil {
				...
				if err := c.serverConn.Send(Auth, AuthMessagePayload{
					Response: authResponse,
					Client:   c.Authenticator.Client,
					User:     c.Authenticator.Username,
				}); err != nil {
					c.Logf("error writing to client socket: %v", err)
					done <- err
					return
				}
	...
	for {
		...
		case MarcoMessagePayload:
			if ch, ok := c.Subscriptions[Marco]; ok {
				...
				if err := c.serverConn.Send(Polo, nil); err != nil {
					c.Logf("unable to ping: %v", err)
				}
			}
